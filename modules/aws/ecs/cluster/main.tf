...
resource "aws_ecs_capacity_provider" "client_cp" {
  count = var.client_auto_scaling_group_arn != null ? 1 : 0

  name = "client-cp-${local.name}"

  auto_scaling_group_provider {
    auto_scaling_group_arn         = var.client_auto_scaling_group_arn
...
resource "aws_ecs_cluster_capacity_providers" "this" {
  ...
  try(aws_ecs_capacity_provider.client_cp[0].name, null),
