...
dependency "client_service" {
  config_path = "../../ecs/service/client"
...
inputs = {
  name         = "client-${local.env_vars.locals.project_name}"
  project_name = local.env_vars.locals.project_name
  environment  = local.env_vars.locals.environment

  cluster_name = dependency.cluster.outputs.name
  service_name = dependency.client_service.outputs.name

  alb_listener_arn        = dependency.alb.outputs.https_listener_arn
  blue_target_group_name  = dependency.alb.outputs.blue_client_tg_arn
  green_target_group_name = dependency.alb.outputs.green_client_tg_arn
}
